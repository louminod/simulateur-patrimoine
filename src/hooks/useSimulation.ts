...
function computeCreditEffort(
  config: SCPICreditConfig,
  loanPayment: number,
  insuranceRate: number,
): { effort: number; monthlyPayment: number; monthlyDividend: number } {
...
export function computeSimulation(
  ...
  if (scpiCredit.enabled && creditEffortData) livretConfigs.push({ initialCapital: scpiCredit.downPayment, monthlyContribution: creditEffortData.effort });
  ...
  const chartData = Array.from({ length: months + 1 }, (_, i) => {
    ...
    if (scpiCredit.enabled && creditEffortData) totalInvestedAtMonth += scpiCredit.downPayment + creditEffortData.effort * i;
  ...
  let passiveIncome = 0;
  const scpiSim = sims.find((s) => s.type === "scpi");
  if (scpiSim) passiveIncome += scpiSim.result.capital * (scpi.rate / 100) / 12;
  const creditSim = sims.find((s) => s.type === "scpi-credit");
  if (creditSim) passiveIncome += creditSim.result.capital * (scpiCredit.rate / 100) / 12;
...
export function useSimulation(
  ...
  return useMemo(
    () => computeSimulation(scpi, scpiCredit, av, per, years),
